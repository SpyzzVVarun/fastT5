...
#     attention_mask_dec = torch.ones(batch_size, dec_seq_length, dtype=torch.int64)
    input_ids_dec = torch.tensor(np.ones((batch_size, enc_seq_length), dtype=np.int64))
    attention_mask_dec = torch.tensor(np.ones((batch_size, enc_seq_length), dtype=np.int64))
